...
# selección
def seleccionar_especies_random(seq, random_size=4):
    np.random.seed(0)
    return np.random.choice(np.unique(seq), size=4)

def seleccionar_especies(path_supplementary_table, 
                         output_name='1.1 especies_seleccionadas.txt',
                         guardar=True):
    df = pd.read_csv(path_supplementary_table, low_memory=False)
    lista_especies = list(seleccionar_especies_random(df['species'], 
                                                      random_size=4))
    print('Especies seleccionadas aleatoriamente:')
    print(selected_species := '\n'.join(lista_especies))
    os.makedirs('../data/output', exist_ok=True)
    if guardar:
        with open(f'../data/output/{output_name}', 'w') as f:
...
def filtrar_especies(df, lista_especies, guardar=True):
    ...
    if guardar:
        df.to_csv('../data/output/1.2 Supplementary_Table_1_short_by_species.csv', index=False)
    return df
...
def metadatos(fasta_input, df, guardar=True):
    ...
    if guardar:
        meta.to_csv('../data/output/1.3 labels.tsv', index=False, sep='\t')
    return meta
...
def secuencias(secuencias, guardar=True):
    ...
    if guardar:
        fasta.to_csv('../data/output/1.4 samples.tsv', index=False, sep='\t')
    return fasta
#%%
if __name__ == '__main__':
    # selección
    path_supplementary_table = '../data/input/1. Datos iniciales/Supplememtary_Table_1.csv'
    df = pd.read_csv(path_supplementary_table, low_memory=False)
